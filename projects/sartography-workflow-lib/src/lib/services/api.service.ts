...
// TODO: this isnt used anywhere  but fileMetaID is definitely wrong
  ...
  /** Get Task Events */
  getTaskEvents(action?: TaskAction, noNulls?: boolean, studyId?: number, workflowId?: number): Observable<TaskEvent[]> {
    const url = this.apiRoot + this.endpoints.taskEvents;
    let httpParams = new HttpParams();
    if (action)  { httpParams = httpParams.set('action', action); }
    if (noNulls) { httpParams = httpParams.set('nonulls', noNulls.toString())}
